...
function showOrderProducts(data) {
    ...
    total.innerHTML = `Total Price <i class="fa-solid fa-dollar-sign"></i>${data.totalPrice}`
    ...
    for (r of data.products) {
        ...
        fetchEndpoint(r.endpoint)
            .then((result) => {
                ...
                productName.innerHTML = `${result.title} <span style="float:right;"><i class="fa-solid fa-dollar-sign"></i>${result.price}</span>`;
