    ...
    for (r of data.products) {
        ...
        fetchEndpoint(r.endpoint)
            .then((result) => {
                ...
                document.getElementById('orderHistoryItemContainer').appendChild(cancelBtn)
            });
    }

    let recap = document.createElement('div');
    recap.style.marginBottom = `1em`;
    recap.innerHTML = `<h2 style='font-size:unset !important;'>Order Total: $${data.totalPrice}</h2> `
