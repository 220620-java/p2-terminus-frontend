...
function showOrderProducts(data) {
    ...
    for (r of data.products) {
        ...
        fetchEndpoint(r.endpoint)
            .then((result) => {
                ...
                let p2 = document.createElement('p')
                p2.innerHTML = `<b>Total Price $ ${result.price}</b>`;
    ...
    totalPrice.style.marginTop = `1em`;
    totalPrice.innerHTML = `$${data.orderPrice}`
    document.getElementById('orderHistoryItemContainer').appendChild(totalPrice);
